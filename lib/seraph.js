...
Seraph.prototype._queryRaw = function(query, params, contents, callback) {
	...
	if (['rest', 'graph'].indexOf(contents) === -1) {
		return callback(new Error('Valid content types: rest, graph'));
	}
	...
	this.call(op, function(err, result) {
		if (err || result.errors[0]) {
			...
			result.data = result.data.map(function(row) { return row[contents]; });
...
Seraph.prototype.queryRaw = function(query, params, callback) {
	...
	this._queryRaw(query, params, 'rest', callback);
};
